...
function Bro(bundleFile) {
  ...
  function bundlePreprocessor(config) {
    ...
    return function(content, file, done) {
      ...
      b.deferredBundle(function(err, content) {
        ...
        b._builtOnce = isAutWatchActive(config);
  ...
  function isAutWatchActive(config) {
    var autoWatch = config.autoWatch;
    var intellijWatch = (config.browserify || {}).intellij;
    return autoWatch || intellijWatch;
  }
